...
/** Epoch ms for a calendar date read from the leading YYYY-MM-DD of a raw string. */
function datePart(raw: unknown): { y: number; m: number; d: number } | null {
...
export function verifiedLocalDateWindow(
  ...
  const from = datePart(validFrom);
  const to = datePart(validTo);
  if (!from) return { issue: "valid_from is missing or has no calendar date" };
  if (!to) return { issue: "valid_to is missing or has no calendar date" };
 ...
 * Missing dates never make an offer expired or upcoming. Unparseable
 * timestamps are treated conservatively (never fresh).
 */
export function freshness(offer: Pick<Offer, "observedAt" | "startsAt" | "expiresAt">, now: Date): Freshness {
  ...
  if (offer.startsAt !== null) {
    ...
    if (Number.isNaN(starts) || starts > nowMs) return "upcoming";
  }
  const observed = Date.parse(offer.observedAt);
  if (Number.isNaN(observed) || nowMs - observed > STALE_AFTER_MS) return "stale";
  return "fresh";
}
